...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d origin_to_center = ray_origin - sphere_center;
    double a = ray_direction.dot(ray_direction);
    double b = 2 * origin_to_center.dot(ray_direction);
    double c = origin_to_center.dot(origin_to_center) - sphere_radius * sphere_radius;
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d neg_edge_u = -pgram_u;
    Vector3d neg_edge_v = -pgram_v;

    Vector3d ray_origin_vec = pgram_origin - ray_origin;

    Matrix3d M;

    M << ray_direction, neg_edge_u, neg_edge_v;
    ...
    Vector3d t = M.inverse() * ray_origin_vec;

    // TODO set the correct intersection point, update p and N to the correct values
    if (t(0) < 0 || t(1) < 0 || t(2) < 0 || t(1) > 1 || t(2) > 1)
    {
        return -1;
    }
    else
    {
        p = ray_origin + t(0) * ray_direction;
